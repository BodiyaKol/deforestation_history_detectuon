"""tests/test_pipeline.py — Юніт- та інтеграційні тести для SVD + regression."""
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
from main_logic_SVD.change_detection import compute_regression_changes
...
def _inject(X, H, W, r0, r1, c0, c1, t0, T, ndvi=0.03, seed=42):
    ...
    flat = gt.flatten()
    for t in range(t0, T):
        Xm[flat, t] = ndvi + rng.normal(0, 0.005, flat.sum())
    return Xm, gt


def t1_svd_baseline_clean():
    X, _ = _scene(noise=0.005)
    L, S, sigma, k = compute_svd_background(X, variance_threshold=0.999)
    v = np.abs(S).max()
    assert v < 0.05, f"max|S|={v:.4f}>0.05 k={k}"
    print(f"  [PASS] svd_baseline_clean       max|S|={v:.5f}  k={k}")


def t2_svd_numpy_match():
    X, _ = _scene(H=30, W=30, T=10)
    L_ours, _, _, k = compute_svd_background(X, variance_threshold=0.99)
    U, s, Vt = np.linalg.svd(X, full_matrices=False)
    s2c = np.cumsum(s ** 2) / np.sum(s ** 2)
    kr = int(np.searchsorted(s2c, 0.99)) + 1
...
def t3_regression_clean():
    X, _ = _scene(noise=0.005)
    H, W, T = 60, 60, 20
    fm, nfm = build_forest_masks(X, H, W)
    am, Z, ct = compute_regression_changes(X, fm, nfm, H, W, baseline_window=3, verbose=False)
    fpr = am[fm.flatten(), :].mean()
    assert fpr < 0.03, f"FPR={fpr:.3%}>=3%"
    print(f"  [PASS] regression_clean        FPR={fpr:.3%}")


def t4_regression_gradual():
    H, W, T = 50, 50, 20
    X, _ = _scene(H=H, W=W, T=T)
    gt = np.zeros((H, W), dtype=bool)
    gt[15:25, 15:25] = True
    flat = gt.flatten()
    ...
    for t in range(T // 2, T):
        p = (t - T // 2) / max(1, T - T // 2)
        Xm[flat, t] = (0.65 - p * 0.50) + rng.normal(0, 0.008, flat.sum())
    fm, nfm = build_forest_masks(Xm, H, W)
    am, Z, ct = compute_regression_changes(Xm, fm, nfm, H, W, baseline_window=3, verbose=False)
    det = am.any(axis=1).reshape(H, W)
    rec = (det & gt & fm).sum() / ((gt & fm).sum() + 1e-8)
    assert rec >= 0.50, f"gradual recall={rec:.2%}<50%"
    print(f"  [PASS] regression_gradual      recall={rec:.2%}")


UNIT_TESTS = [
    t1_svd_baseline_clean,
    t2_svd_numpy_match,
    t3_regression_clean,
    t4_regression_gradual,
]


def _metrics(detected, gt_3d, event_start):
    gt_any = gt_3d[:, :, event_start:].any(axis=2)
    TP = (detected & gt_any).sum()
    FP = (detected & ~gt_any).sum()
    FN = (~detected & gt_any).sum()
    p = TP / (TP + FP + 1e-8)
    r = TP / (TP + FN + 1e-8)
    f1 = 2 * p * r / (p + r + 1e-8)
...
def _plot_detection(X, L, Z, am_clean, gt_3d, H, W, T, dates, sigma, k, out_dir, name):
    plot_diagnostics(sigma, X, L, Z, am_clean, H, W, dates, k, out_dir, title_suffix=name)
    ...
    axes[0].set_title("Кадр 0 — початок")
    axes[0].axis("off")

    axes[1].imshow(X[:, -1].reshape(H, W), cmap="YlGn", vmin=0, vmax=0.8)
    axes[1].set_title(f"Кадр {T-1} — кінець")
    axes[1].axis("off")
    ...
    overlay[:, :, 1] = gt_any.astype(float) * 0.7
    overlay[:, :, 0] = det_any.astype(float) * 0.9
    axes[3].imshow(overlay)
    axes[3].set_title("GT / Detected")
    axes[3].axis("off")
...
def run_one(image_dir, results_base):
    ...
    fm, nfm = build_forest_masks(X, H, W)
    am, Z, change_time = compute_regression_changes(X, fm, nfm, H, W, baseline_window=3, verbose=False)
    L = np.repeat(X[:, 0:1], X.shape[1], axis=1)
    sigma = np.array([])
    k = 0
    ...
    _plot_detection(X, L, Z, am_cl, gt_3d, H, W, T, dates, sigma, k, out_dir, "SVD+Regression")
...
def run_integration(images_dir, results_dir):
    ...
    if not dirs:
        ...
        print("    Запусти: python tests/generate_test_images.py --all")
        return False

    results_base = Path(results_dir) / "regression"
    print("\n" + "═"*55)
    print(f"  INTEGRATION TESTS  [SVD + Regression] ({len(dirs)} сценаріїв)")
    print("═"*55)
    all_m = []
    for d in dirs:
        all_m.append(run_one(d, results_base))
    print("\n" + "─"*55)
    ...
    print("─"*55)
    summary = ["SUMMARY", "="*50] + [
        f"{m['name']:<32} P={m['precision']:.3f} R={m['recall']:.3f} F1={m['f1']:.3f}"
        for m in all_m
    ]
    results_base.mkdir(parents=True, exist_ok=True)
    (results_base / "summary.txt").write_text("\n".join(summary))
...
def main():
    ...
    if run_i:
        if not run_integration(args.images_dir, args.results_dir):
            ok = False
